...
Deno.serve(async (req) => {
  ...
  try {
    ...
    organizerId = url.searchParams.get("organizerId");

    if (!organizerId && req.method === "POST") {
    ...
    return json({
      ...
      accountId: organizer.stripe_account_id,
    });
  } catch (err: any) {
    console.error("Connect account-status error:", err);
    return json({ error: err.message || "Failed to get account status" }, 500);
  }
});
