...
import {
  ...
  Building2, ArrowRightLeft, Shield
} from "lucide-react";
...
export default function ProfilePage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Manage your tickets, saved events, and profile.
        </p>
  ...
  const menuItems = [
    ...
    { icon: Heart, label: "Saved Events", path: "/saved" },
    ...
    <div className="pb-20 md:pb-8 max-w-4xl mx-auto w-full">
      <div className="px-4 py-6">
        ...
        {userOrganizers && userOrganizers.length > 0 ? (
          <Link
            to="/dashboard"
            className="flex items-center gap-3 p-3 rounded-xl hover:bg-muted transition-colors mb-6"
          >
            <Megaphone className="w-5 h-5 text-primary" />
            <span className="flex-1 text-sm font-medium text-foreground">Organizer Dashboard</span>
            <ChevronRight className="w-4 h-4 text-muted-foreground" />
          </Link>
        ) : (
          <Link
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action is permanent. All your data including tickets, saved events, and profile will be permanently deleted.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-4 pt-2">
          ...
          <p className="text-xs text-muted-foreground text-center">
            You'll need to confirm the change from both email addresses.
          </p>
        </div>
      </DialogContent>
    </Dialog>
...
function EditNameDialog({ currentName, userId, onSuccess }: {
  currentName: string;
  userId: string;
  onSuccess: () => void;
}) {
